    ...
    if(status == Solution::OPTIMAL || status == Solution::SUBOPTIMAL) {
        ...
        if(sol)
            for(auto it = varOrder_.begin(); it != varOrder_.end(); ++it) {
                auto var = lp_->getVariable(it->first);
                sol->addVariable(var, glp_mip_col_val(model_, it->second));
            }
    }
    if(sol)
...
void GLPK::addVar(Variable *v) {
    ...
    varOrder_.insert({v->getID(), (int)varOrder_.size()+1});
    glp_set_col_name(model_, varOrder_[v->getID()], v->getID().c_str());
    glp_set_col_bnds(model_, varOrder_[v->getID()], GLP_DB, v->getLowerBound(), v->getUpperBound());
    switch(v->getType()) {
...
void GLPK::addLinearConstraint(LinearConstraint *c) {
    ...
    constOrder_.insert({c->getID(), (int)constOrder_.size()+1});
    glp_set_row_name(model_, constOrder_[c->getID()], c->getID().c_str());
